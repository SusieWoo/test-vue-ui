...
export default {
  ...
  updateTeamInfo(state, team) {
    let index = state.teamlist.findIndex(item => {
  ...
  updateSingleTeamMsgReads(state, obj) {
    ...
    // 更新已读未读账号列表
    let unreadAccounts = state.teamMsgReadsDetail.unreadAccounts
    let findIndex = unreadAccounts.findIndex(account => account === obj.account)
    if (findIndex >= 0) {
