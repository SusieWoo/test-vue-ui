      ...
      <el-form ref="loginForm"
        ...
        <el-form-item v-if="showVerifyImg" prop="verifyCode">
          ...
          <el-input ref="verifyCode"
                    ...
                    autocomplete="off"
                    style="width: 54%"
                    :placeholder="$t('login.picCode')" />
          <img v-if="verifyImg" :src="verifyImg" class="verifyImg" @click="changeVerify" />
          <el-button class="smscodeBtn verifyCode" type="primary" @click.native.prevent="changeVerify">{{ $t('login.cannotSee') }}</el-button>
        </el-form-item>
...
import { getVerifyCode, isShowVerifyCode, getMsgCode, resetPassword , getMenu } from '@/api/users'
...
export default class extends Vue {
  private validateUsername = (rule: any, value: string, callback: Function) => {
    if (!value) {
      return callback(new Error(lang.t('login.userNameEmpty')))
    } else {
      callback()
    }
  }
  private validatePassword = (rule: any, value: string, callback: Function) => {
    if (!value) {
      callback(new Error(lang.t('login.passwordEmpty')))
    } else {
      callback()
    }
  }
  ...
  @Prop({ default: () => [] }) private roleArr!: any
  @Prop({ default: () => lang.t('login.getCode') }) private msgTitle!: any
    ...
    await getMsgCode(this.loginForm).then(res => {
      ...
      _this.timePromise = setInterval(function() {
        if (second <= 0) {
          ...
          _this.msgTitle = lang.t('login.reSend')
          _this.disableMsgCode = false
        } else {
          _this.msgTitle = second + lang.t('login.reSendTime')
          _this.disableMsgCode = true
          second--
        }
      }, 1000, 100)
      _this.$message({ message: lang.t('login.codeSendSuccess'), type: 'success', duration: 5 * 1000 })
    ...
    (this.$refs.loginForm as ElForm).validate(async (valid: boolean) => {
      if (valid) {
        ...
        if (UserModule.isFirstLogin === 1 || UserModule.isShowVerifyCode === true) {
          ...
          //
          // let _this = this
          // let allRoute: any = _this.$router
          // let index = allRoute.options.routes.findIndex((state: any) => {
          //   let result = false
          //   if (state.children && (!state.meta || !state.meta.hidden)) {
          //     for (let i = 0; i < state.children.length; i++) {
          //       if (!state.children[i].hidden) {
          //         result = _this.roleArr.includes(state.children[i].roleId)
          //         break
          //       }
          //     }
          //     return result
          ...
          // })
          // if (index < 0) {
          //   _this.$message({ message: lang.t('login.noPermission'), type: 'error' })
          // } else {
          //   _this.$router.push({ path: allRoute.options.routes[index].path })
          // }
        }
