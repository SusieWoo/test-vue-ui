<template>
    <div class="message-add">
        <el-form ref="dataForm"
                 ...
                 label-width="120px"
                 :model="row">
            <el-form-item :label="$t('message.describe')" prop="describe">
            ...
            <el-form-item :label="$t('message.targetUrl')" prop="targetUrl">
            ...
            <div v-if="row.targetUser === 1" style="display: flex; flex-direction: row; padding-left:50px;">
                <el-form-item :label="$t('message.carSeries')" prop="seriseId" >
                    <el-select
                            v-model="seriseId"
                            class="search-item"
                ...
                <el-form-item :label="$t('message.carType')" prop="modelId">
                    <el-select
                            v-model="row.modelId"
                            class="search-item"
                            :placeholder="$t('common.select')"
            ...
            <!-- TODO 左右移动选择框-->
            <div v-if="row.targetUser === 2" style="display: flex; flex-direction: row; padding-left:50px;">
                <el-form-item :label="$t('message.carSeries')" prop="requiredYes">
                    <el-select
            ...
            <el-form-item :label="$t('message.noticeType')">
                <el-radio v-model="row.noticeType" :label="0">{{$t('message.appMsg')}}</el-radio>
            </el-form-item>
            ...
            <el-form-item v-if="row.sendType === 1" prop="sendTime">
                <el-date-picker
            ...
            <el-form-item :label="$t('message.validityTime')">
                <el-radio v-model="row.validity" label=0>{{$t('message.noSave')}}</el-radio>
            </el-form-item>
            <el-form-item>
                <el-radio v-model="row.validity" label=1>{{$t('message.yesSave')}}</el-radio>
                <el-select
                        v-model="row.saveDays"
                        class="search-item"
                        :placeholder="$t('common.select')"
                >
                    <el-option
            ...
            <el-form-item>
                <el-button type="primary" @click.native="sendMsg('dataForm')">{{$t('message.sendMsg')}}
                </el-button>
            </el-form-item>
        </el-form>
    </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            async getBrandList() {
                const re = await brandList(this.row);
                this.brandId = re.data[0].brandId;
                this.getCarSeriesList(this.brandId);
                // const res = await carSeriesList({ brandId: this.brandId });
            ...
            async getCarTypeList(id) {
                ...
                this.row.seriseId = obj.seriseName;
            ...
            async getMessageDetail(msgId) {
                ...
                this.seriseId = obj.seriseId;
                // 车型
                this.getCarTypeList(this.seriseId);
